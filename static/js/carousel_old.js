function carousel() {
    return {
        ...
        setupCarousel() {
            ...
            // Dimensions du conteneur visible
            ...
            // Détection mobile ou paysage (adapte selon préférence)
            const isMobile = window.innerWidth <= 768 || window.innerHeight > window.innerWidth;

            if (isMobile) {
                // Sur mobile : la hauteur de la carte est maximale possible
                this.cardHeight = this.containerHeight;
                this.cardWidth = this.cardHeight * cardRatio;
                this.gap = 8; // petit espace entre les cartes
            } else {
                // Desktop/tablette paysage
                // Carte la plus grande possible sans dépasser le wrapper
                const maxWidth = this.containerHeight * cardRatio;
                if (maxWidth * 2 + this.gap > this.containerWidth) {
                    this.cardWidth = Math.min(
                        (this.containerWidth - (this.gap * 2)) / 2.5,
                        maxWidth
                    );
                    this.cardHeight = this.cardWidth / cardRatio;
                } else {
                    this.cardHeight = this.containerHeight;
                    this.cardWidth = this.cardHeight * cardRatio;
                }
                this.gap = 30; // ou ce que tu utilisais déjà
            }

            this.cardTotalWidth = this.cardWidth + this.gap;

            // Appliquer dimensions à toutes les cartes
            ...
            // Centrage : afficher la première carte pleine au centre
            this.offset = (this.containerWidth - this.cardWidth) / 2 - 8;
        ...
        next() {
            this.currentIndex++;
            this.offset -= this.cardTotalWidth;
            this.updatePosition(true);
            setTimeout(() => { this.checkInfiniteScroll(); }, 500);
        },
        ...
        handleTouchMove(e) {
            ...
            this.$refs.track.style.transform = `translateX(${this.offset + walk
                }px)`;
        },
